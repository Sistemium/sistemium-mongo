...
async function archiveCreate(data, name, authId) {
  const $set = { name, data, authId };
...
export function delHandler(model) {
  ...
  return async ctx => {
    ...
    debug('DELETE', path, authId);

    const data = await model.findOne({ id });

    ctx.assert(data, 404);

    await archiveCreate(data, model.schema.options.collection, authId);
    await data.delete();
