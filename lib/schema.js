...
export default class ModelSchema {
  ...
  normalizeItem(item, defaults = {}) {
    const { schema: { tree } } = this;
    const all = mapValues(tree, (keySchema, key) => item[key] || defaults[key]);
...
function $updateOne(item, id, upsert = true) {
  ...
  const cts = new Date();
  const { mergeBy } = this;
  const toOmit = ['_id', 'ts', 'cts', 'authId', ...mergeBy];
  ...
  const update = {
    ...
    $unset,
    $setOnInsert: { cts, id, authId },
  };

  if (!authId) {
    delete update.$setOnInsert.authId;
  }
