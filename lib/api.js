...
export function postHandler(model) {
  return async ctx => {
    ...
    debug('POST', path, data.length, 'records', creatorAuthId);

    const $in = await model.merge(data.map(item => model.normalizeItem(item, {}, { creatorAuthId })));
    let merged;

    if (mergeById(model)) {
      merged = $in.length ? await model.findAll({ id: { $in } }) : [];
    } else {
      merged = await mapSeries(data, async item => {
        const keys = lo.pick(item, model.mergeBy);
        return model.findOne(keys);
      });
    }
